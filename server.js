...
if(currentUser)
{
  currentUser.isMute = !currentUser.isMute;
	...
	socket.on('VIDEO_PLAY', function(){
		socket.broadcast.emit('PLAY_VIDEO_CLIENT')
	})

	socket.on('VIDEO_PAUSE', function(){
		socket.broadcast.emit('PAUSE_VIDEO_CLIENT')
	})

	socket.on('MUSIC_PLAY', function(){
		socket.broadcast.emit('PLAY_MUSIC_CLIENT')
	})

	socket.on('MUSIC_PAUSE', function(){
		socket.broadcast.emit('PAUSE_MUSIC_CLIENT')
	})
	...
	socket.on('CHAIR',function(data){
		...
		console.log("Character siitng on chari "+pack.chair)
		socket.broadcast.emit('CHAIR_FILLED', pack.chair);
	})

	socket.on('CHAIR_OUT',function(data){

		var pack = JSON.parse(data);

		console.log("Character siitng on chari "+pack.chair)
		socket.broadcast.emit('CHAIR_FILLED_OUT', pack.chair);
	})

	socket.on('TOGGLE_OBJECT', function(data){
		var pack = JSON.parse(data)
